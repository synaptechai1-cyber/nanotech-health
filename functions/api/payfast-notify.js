...
import { buildIncomingSignature, PLAN_PRICING } from '../_shared/payfast.js'
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    const { signature: expectedSignature, paramString } = buildIncomingSignature(pfData, env.PAYFAST_PASSPHRASE, { createHash })
    if (expectedSignature !== pfData.signature) {
      await logITN(env, 'signature_mismatch', `expected=${expectedSignature} got=${pfData.signature} paramString=${paramString}`)
